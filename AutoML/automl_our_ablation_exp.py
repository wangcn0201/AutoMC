...
class AutoMLOur_AblationEXP(object):
	def __init__(self, config_path, task_name, task_info):
		super(AutoMLOur_AblationEXP, self).__init__()
		...
		self.k_model = KnowledgeModel(config, self.cstrategies_path, self.knowledge_path, self.logger, EXP=False).cuda()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/AutoML_Our_Ablation_EXP-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
if __name__ == '__main__':
	...
	nas_obj = AutoMLOur_AblationEXP(config_path, task_name, task_info)
	nas_obj.main()
'''