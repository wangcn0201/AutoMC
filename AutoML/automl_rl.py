...
class AutoMLRl(object):
	def __init__(self, config_path, task_name, task_info):
		super(AutoMLRl, self).__init__()
		...
		self.model = RNNModel(self.HyperParameters, self.cstartegies, self.action_list).cuda()
		self.cuda = True
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/AutoML_RL-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
if __name__ == '__main__':
	...
	nas_obj = AutoMLRl(config_path, task_name, task_info)
	nas_obj.main()
'''