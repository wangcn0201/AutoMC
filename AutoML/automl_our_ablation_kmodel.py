...
from automc.ParetoModel_kmodel import ParetoModel_kmodel


class AutoMLOur_AblationKModel(object):
	def __init__(self, config_path, task_name, task_info):
		super(AutoMLOur_AblationKModel, self).__init__()
		...
		self.p_model = ParetoModel_kmodel(len(self.cstartegies), task_array, self.target_compression_rate).cuda()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/AutoML_Our_Ablation_KModel-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
if __name__ == '__main__':
	...
	nas_obj = AutoMLOur_AblationKModel(config_path, task_name, task_info)
	nas_obj.main()
'''