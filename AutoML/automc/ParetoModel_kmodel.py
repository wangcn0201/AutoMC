...
class ParetoModel_kmodel(torch.nn.Module):
	def __init__(self, cstartegies_num, task_array, target_compression_rate, embedding_dim=32, controller_dim=32, output_dim=2, hidden_dim=32, num_layers=2, dropout=0.5):
		super(ParetoModel_kmodel, self).__init__()

		self.target_compression_rate = target_compression_rate

		self.cstartegy_embeddings = nn.Embedding(cstartegies_num+1, embedding_dim)
	...
	def ProcessPreSequences(self, pre_sequences):
		...
		normed_pre_sequences_embedding = self.cstartegy_embeddings(torch.tensor(normed_pre_sequences).long().cuda()) # batch, seq, feature
	...
	def main(self, pre_sequences, next_cstrategies, history_scheme_scores, optimal_num):
		...
		next_cstrategy_embeddings = self.cstartegy_embeddings(torch.Tensor(np.array(next_cstrategies).reshape(-1,1)).long().cuda()).squeeze(1)
