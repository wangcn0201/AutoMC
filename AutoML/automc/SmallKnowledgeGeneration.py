...
class SmallKnowledgeGeneration(object):
	def __init__(self, space, cstartegies, knowledge_path, logger):
		super(SmallKnowledgeGeneration, self).__init__()
	...
	def GenerateKG(self, space, cstartegies, knowledge_path):
		...
		for i in range(1, len(kb_exp_info)):
			...
			if calg not in list(self.space.keys()):
				continue
		...
		for i in range(1, len(kb_calg_info)):
			...
			if calg in list(self.space.keys()):
				calg_id = entity_mapping["calg: "+str(calg)]
		...
		with open(knowledge_path + '/small_kg_reverse_entity_mapping.pkl', 'wb') as file:
			pickle.dump(reverse_entity_mapping, file)
		with open(knowledge_path + '/small_kg_entity_mapping.pkl', 'wb') as file:
			pickle.dump(entity_mapping, file)
		with open(knowledge_path + '/small_kg_relation_mapping.pkl', 'wb') as file:
			pickle.dump(relation_mapping, file)
		with open(knowledge_path + '/small_kg_kg_info.pkl', 'wb') as file:
	...
	def GenerateEXP(self, cstartegies, entity_mapping, complete_cstartegies, knowledge_path):
		...
		for i in range(1, len(kb_exp_info)):
			...
			if cstartegy_name not in complete_cstartegies:
				continue
		...
		with open(knowledge_path + '/small_exp_infos_1.pkl', 'wb') as file:
			pickle.dump(exp_infos_1, file)
		with open(knowledge_path + '/small_exp_infos_2.pkl', 'wb') as file:
