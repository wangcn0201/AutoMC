...
class AutoMLRandom(object):
	def __init__(self, config_path, task_name, task_info):
		super(AutoMLRandom, self).__init__()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + 'AutoML_Random-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
	...
	def get_scheme_code(self):
		scheme_code = []
		calg_num = random.randint(1, self.max_calg_num)
		for i in range(calg_num):
			calg = random.sample(self.calgs, 1)[0]
			calg_hpo = {}
			calg_hpo_dict = self.space[calg]
			for hpo in calg_hpo_dict.keys():
				calg_hpo[hpo] = random.sample(calg_hpo_dict[hpo],1)[0]
			scheme_code.append(list([calg, calg_hpo]))
...
if __name__ == '__main__':
	...
	nas_obj = AutoMLRandom(config_path, task_name, task_info)
	nas_obj.main()
'''