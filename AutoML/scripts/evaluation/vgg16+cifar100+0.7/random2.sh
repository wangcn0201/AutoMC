#!/bin/sh

DIR=$(cd `dirname $0`; pwd)
FILENAME=${0##*/}
ALGNAME=${FILENAME%.*}
CONFIGFILENAME="${DIR}/config.ini"
TASKNAME=${DIR##*/}

source $CONFIGFILENAME

NUMDEC=0.71604019266793
SCHEMECODE="[['prune_C4', {'HP11': 5, 'HP10': 0.8, 'HP2': 0.5}], ['prune_C1', {'HP5': 0.3, 'HP4': 6, 'HP3': 10, 'HP2': 1.0, 'HP1': 0.2}], ['prune_C5', {'HP15': 1, 'HP14': 1.5, 'HP13': 'l1norm', 'HP12': 'P3', 'HP2': 0.9, 'HP1': 0.8}]]"

nohup sh scripts/evaluation/evaluation_general.sh $NUMDEC $TASKNAME $ALGNAME "$SCHEMECODE" $1 $REALTASKNAME > ./logs/log_evaluation_${TASKNAME}_${ALGNAME}_${1}.log &
