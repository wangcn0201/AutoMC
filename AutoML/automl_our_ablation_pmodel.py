...
from automc.ParetoModel_pmodel import ParetoModel_pmodel


class AutoMLOur_AblationPModel(object):
	def __init__(self, config_path, task_name, task_info):
		super(AutoMLOur_AblationPModel, self).__init__()
		...
		self.action_list = list(self.HyperParameters.keys())
		...
		self.model = ParetoModel_pmodel(cstrategy_embeddings, self.HyperParameters, self.cstartegies, self.action_list).cuda()
		self.cuda = True
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/AutoML_Our_Ablation_PModel-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
	...
	def main(self):
		...
		search_time = 0.0
		self.model.train()
		history, baseline, total_loss = [], None, 0
		while search_time < self.automl_search_time_s:
			self.codes += 1

			# sample graphnas
			self.logger.info('@ recommend model_code......')
			scheme_code_list, log_probs, entropies = self.model.sample(with_details=True)
			# calculate reward
			scheme_code = scheme_code_list[0]
			...
			if scheme_score[1] >= self.target_compression_rate:
				rewards = scheme_score[0]*0.1+scheme_score[1]
			else:
				rewards = scheme_score[0]*0.08+scheme_score[1]
			self.logger.info('@ rewards: %s', str(rewards))
			...
			# policy loss
			self.logger.info('@ policy loss updating......')
			loss = -log_probs * adv
			loss = loss.sum()  # or loss.mean()
			self.logger.info('@ loss of the RNN controller: %f', loss)
			# update
			self.optimizer.zero_grad()
			loss.backward()
			self.optimizer.step()
			total_loss += utils.to_item(loss.data)
			self.logger.info('@ total_loss of the RNN controller: %f', total_loss)
...
if __name__ == '__main__':
	...
	nas_obj = AutoMLOur_AblationPModel(config_path, task_name, task_info)
	nas_obj.main()
'''